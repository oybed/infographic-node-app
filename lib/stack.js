	...
	request(options, function (error, response) {
		...
		if (!error && response.statusCode >= 200 && response.statusCode < 300) {
			sendResponse(cb, 
				     ...
				     '', 
			             'https://' + AnsibleTowerUrl + '/#/management_jobs/' + response.body.job);
...
function processStack(body, cb){
	...
	if (body && body.projectName && body.getUrl) {
		...
		if (('node' in params) && 
		    (params['node'] == "true")) { 
			launchRequest(params, response);
		} else {
			sendResponse(cb, '', 'No Valid Technologies selected. Please check your selections and try again.');
		}
	} else {
		sendResponse(cb, '', 'No extra_vars specified.');
	}
